...
__global__ void sum_reduction(int *v, int *v_r) {
  __shared__ int partial_sum[SHMEM_SIZE];
	...
	// This extra calculation step is causing much more latency even though a lesser number of cores are being called over all
	// it might be due to the extra memory allocation (i)
	//also note that modulo operator is expensive, try to avoid using it
